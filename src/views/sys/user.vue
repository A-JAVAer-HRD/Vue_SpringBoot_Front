  ...
  <div>
    ...
    <el-dialog @close="clearFrom" :title="title" :visible.sync="dialogFormVisible">
      <el-form :model="userForm" ref="userFormRef" :rules="rules">
        ...
        <el-form-item label="登录密码" v-if="userForm.id == null || userForm.id == undefined" prop="password" :label-width="formLabelWidth">
    ...
    methods: {
      ...
      saveUser(){
        ...
        this.$refs.userFormRef.validate((valid) => {
          if (valid) {
            // 提交请求给后台
            userApi.save(this.userForm).then(response => {
              // 成功提示
              this.$message({
                message: response.message,
                type: 'success'
      ...
      openEditUI(id){
        if(id == null) {
          ...
          this.title = '修改用户';
          // 根据id查询用户数据
          userApi.getUserById(id).then(responce => {
            this.userForm = responce.data;
          });
        }
